...
fn header_kat_path_challenge_packet() {
    ...
    let mut h = Header::new(PacketType::PathChallenge, 0x000000FF, [0x01; 8], [0x02; 8]);
    h.payload_len = 32;
...
fn header_kat_path_response_packet() {
    let mut h = Header::new(PacketType::PathResponse, 0x0000_0100, [0x03; 8], [0x04; 8]);
    h.payload_len = 32;
    ...
    let expected: [u8; HEADER_LEN] = [
        ...
        0x04, 0x01, 0x00, // hop_ttl, reserved
        0x00, 0x20, // payload_len = 32
        0x00, 0x00, 0x00, 0x00, // send_time_ms
    ];
...
fn aead_kat_path_challenge_sealed() {
    ...
    assert!(
        k.open(7, PacketType::PathResponse as u8, &aad, &ct)
            .is_err(),
        "AEAD cross-type open must fail"
    );
}
