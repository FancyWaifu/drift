...
fn cargo_bin(name: &str) -> std::path::PathBuf {
    // In a workspace, CARGO_MANIFEST_DIR is the member crate dir
    // but binaries land in the workspace root target/debug/examples.
...
fn send_and_recv_across_os_processes() {
    ...
    if !recv_bin.exists() || !send_bin.exists() {
        let status = Command::new(env!("CARGO"))
            .args(["build", "-p", "drift", "--examples", "--quiet"])
            .status()
