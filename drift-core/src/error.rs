...
//!     `Replay(u32)` variants, which had zero direct producers
//!     after slices 2 and 4 finished. The flat `AuthFailed`,
//!     `HandshakeExhausted`, `QueueFull`, and `DecodeError`
//!     variants still exist because 30+ unmigrated direct
//!     producers still emit them; follow-up slices migrate
//!     those producers and a final slice drops the last flat
//!     variants. Three consumer match arms updated:
//!     `mod.rs:run_recv_loop_for` (was `UnknownPeer =>`, now
//!     `Peer(_) =>` for the `unknown_peer_drops` metric; was
//!     `Replay(_) =>`, now `Crypto(CryptoError::Replay { .. }) =>`
//!     for the `replays_seen` metric) and
//!     `tests/graceful_migration.rs` (assertion updated to
//!     `matches!(err, DriftError::Peer(_))`).
//!
...
pub enum DriftError {
    ...
    /// produce sites migrated through slices 4a–4f now return
    /// the appropriate `PeerError` variant, which `?` lifts into
    /// `Peer(PeerError)`. Match on `DriftError::Peer(_)` for
    /// "peer / session-not-ready" handling, or destructure into
    /// the inner variant for finer-grained behavior.
    #[error(transparent)]
    Peer(#[from] PeerError),
