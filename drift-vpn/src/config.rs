...
pub(crate) fn parse_bridge_spec(spec: &str) -> Result<(String, [u8; 32])> {
...
"#,
    ...
    fn peer_via_bridges_accepts_array_form() {
        // mtu lives on `[interface]`, not the per-peer table — the
        // older form of this test put `mtu = 1200` under `[[peer]]`
        // which silently parsed before `deny_unknown_fields` landed.
        let mut body = minimal_interface(
            r#"listen = "udp://0.0.0.0:51820"
mtu = 1200"#,
        );
        body.push_str(&format!(
            r#"
...
"#,
    ...
    #[test]
    fn rejects_typo_in_peer_field_name() {
        // Common foot-gun: typing `via-bridges-auto` (dashes)
        // instead of `via_bridges_auto` (underscores) silently
        // got default behavior with no warning, which produced
        // hours of "why doesn't drift-vpn see my bridges?"
        // debugging today. deny_unknown_fields surfaces the typo
...
public_key       = "{peer}"
allowed_ips      = ["10.0.0.2/32"]
via-bridges-auto = "host@{bridge}"
"#,
            peer = PUB_HEX,
            bridge = BRIDGE_PUB_HEX,
        ));
        let err = toml::from_str::<Config>(&body).expect_err("typo'd key should be rejected");
        let msg = format!("{}", err);
        assert!(
            msg.contains("via-bridges-auto") && msg.contains("via_bridges_auto"),
            "error should name both the typo and the canonical key; got: {msg}"
        );
    }

    #[test]
    fn rejects_typo_in_interface_field_name() {
        // Same protection on the `[interface]` table.
        let body = r#"
[interface]
identity_file = "/tmp/test-identity"
adddress = "10.0.0.1/24"
listen = "udp://0.0.0.0:51820"
"#;
        let err =
            toml::from_str::<Config>(body).expect_err("typo'd interface field should be rejected");
        let msg = format!("{}", err);
        assert!(
            msg.contains("adddress"),
