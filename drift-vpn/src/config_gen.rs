...
pub fn init(config_path: &Path, cidr: &str, mtu: u32) -> Result<()> {
    ...
    println!("added [vpn] cidr={} mtu={} to {}", cidr, mtu, config_path.display());
    println!();
...
pub fn gen(config_path: &Path, out_dir: &Path) -> Result<()> {
    ...
    let vpn = doc
        .vpn
        .as_ref()
        .ok_or_else(|| anyhow!("no [vpn] block in drift.toml — run `drift-vpn config init` first"))?;
    ...
    for (name, vh) in &vpn.hosts {
        let host = doc.hosts.get(name).ok_or_else(|| {
            anyhow!("vpn assigns {:?} but [hosts.{}] is missing — run `drift-config validate`", name, name)
        })?;
    ...
    println!();
    println!("wrote {} config{} to {}", written, if written == 1 { "" } else { "s" }, out_dir.display());
    println!();
    println!("deploy each one with:");
    println!("  scp {0}/<host>/config.toml root@<host>:/etc/drift-vpn/config.toml", out_dir.display());
