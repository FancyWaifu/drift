...
`sec/fed-bench/` — two-mode binary (`MODE=listen` / `MODE=send`) for per-scheme throughput. 5-second LAN run between Drift1 (192.0.2.52) and Drift2 (192.0.2.168) gave:
...
| Router (ASUS RT-AX82U V2) | 192.0.2.1 | permanent DRIFT bridge | `ssh admin@192.0.2.1` | armv7l, stock ASUSWRT, binary at `/jffs/drift/drift`. **No scp** — use `cat binary \| ssh admin@192.0.2.1 'cat > /jffs/drift/drift'`. Manual relaunch on reboot. Pubkey `426b1...`. Ports: udp/tcp/ws/tls 51820–51823, h2 51826, h2s 51827, webtransport 51828. |
| Drift1 LXC | 192.0.2.52 | test peer | `ssh root@192.0.2.52` | Proxmox LXC. Small disk — clean up `/tmp/` and `/root/*` after runs. |
| Drift2 LXC | 192.0.2.168 | test peer | `ssh root@192.0.2.168` | |
| Drift3 LXC | 192.0.2.253 | spare (~6.8G free, use for big deps like caddy) | `ssh root@192.0.2.253` | |
| Drift4 LXC | 192.0.2.33 | spare | `ssh root@192.0.2.33` | |
...
### 4.3 Memory layer (`~/.claude/projects/-Users-youruser-drift/memory/`)
