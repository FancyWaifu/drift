...
scp drift admin@192.168.50.1:/opt/sbin/drift
ssh admin@192.168.50.1 'chmod +x /opt/sbin/drift'
```

Verify it runs:

```bash
ssh admin@192.168.50.1 '/opt/sbin/drift --help'
```
...
Port Range:       51820
Local IP:         192.168.50.1   (the router's LAN IP)
Local Port:       51820
Protocol:         UDP
```
...
ssh admin@192.168.50.1 'ps | grep drift'
ssh admin@192.168.50.1 'tail -20 /opt/var/log/drift-bridge.log'
```
...
PUB_BRIDGE=$(ssh admin@192.168.50.1 '/opt/sbin/drift info /opt/etc/drift/bridge.key | grep -oE "[0-9a-f]{64}"')
